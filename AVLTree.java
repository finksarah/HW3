...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        if (value < node.value) {
            node.leftChild = deleteElement(value, node.leftChild); // traverse the left subtree
        } else if (value > node.value) {
            node.rightChild = deleteElement(value, node.rightChild); // traverse the right subtree
        } else {
            if (node.leftChild == null && node.rightChild == null) { // if node has no children return null
                return null;
            }
            else if (node.leftChild == null) {
                return node.rightChild; // if only one node has a child return it
            } else if (node.rightChild == null) {
                return node.leftChild; // if only one node has a child return it
            } else {
                Node successor = minValueNode(node.rightChild); //if both have children find the smallest node in the right subtree
                node.value = successor.value;  // copy successor's value to current node
                node.rightChild = deleteElement(successor.value, node.rightChild); // delete the succesor node recursively
            }
        }

        node.height = 1 + getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild)); // update the height of the current node

        int balanceFactor = getBalanceFactor(node);


        if (balanceFactor > 1) {   //  if balance factor is greater than 1, it is left heavy so rotate that way
        ...
        if (balanceFactor < -1) { // ff balance factor is less than -1, it is right heavy so rotate that way
